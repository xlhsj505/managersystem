	...
	$('#main-tab').on('click','td',function(){  
    	...
    	var td = $(this).index();
		    if (td == 4){
	    		...
	    		$.ajax({
	    			type: "post",
	    			url: "TakeServlet?action=ajaxUpRegisterAndCheckInTimeByIUID",
   					data: {"iUID":IUID, "td":td},
   					dataType: "json",
   					success: function(data){
   						...
   						if (rs != 0){
							obj.text("已报到");
							//obj.setAttribute( "rel ",e.href) 
							//obj.removeAttribute( "href ") 
						}
   					}
	    		});
		    } 
		    if (td == 5 & $(this).parent("tr").find("td:eq(4)").text() == "已报到"){
		    ...
		    } else if(td == 5 & $(this).parent("tr").find("td:eq(4)").text() == "未报到"){
...
<table width="99%" border="0" cellspacing="0" cellpadding="0" id="searchmain">
  <tr>
    <td width="99%" align="left" valign="top">您的位置：签到检录</td>
  </tr>
  <tr>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="search">
  		<tr>
   		 <td width="90%" align="left" valign="middle">
	         <form method="post" action="TakeServlet?action=takeDoSingInRegister">
	         <span>身份证：</span>
	         <input type="text" id="idCard" name="idCard" value="" class="text-word">
	         <span>学校：<select name="schoolName" id="level"> 
  ...
  <tr>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="main-tab">
      <tr>
        ...
        <th align="center" valign="middle" class="borderright">报到时间</th>
        <th align="center" valign="middle" class="borderright">检录时间</th>
        <th align="center" valign="middle" class="borderright">总成绩</th>
        <!-- <th align="center" valign="middle">操作</th>-->
      </tr>
      <c:forEach var="item" items="${sessionScope.pageStudent.pageData }" varStatus="status">
      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
	        ...
	        <td align="center" valign="middle" class="borderright borderbottom">${item.registTime != null ? "已报到" : "<a  target='mainFrame' onFocus='this.blur()' class='add'>未报到</a>"}</td>
	        <td align="center" valign="middle" class="borderright borderbottom">${item.checkInTime != null ? "已检录" : "<a  target='mainFrame' onFocus='this.blur()' class='add'>未检录</a>"}</td>
	        <td align="center" valign="middle" class="borderright borderbottom" contentEditable="true">${item.totalScore }</td>
  ...
  <tr>
    <td align="center" valign="middle" class="fenye">
    	当前&nbsp;${sessionScope.pageStudent.currentPage }&nbsp;/&nbsp;${sessionScope.pageStudent.totalPage }&nbsp;页   &nbsp;&nbsp;
    	<a href="javascript:post('TakeServlet?action=takeDoSingInRegister',
    	{'idCard':'${sessionScope.idCard }', 'schoolName':'${sessionScope.schoolName }', 
    	'currentPage':'1'})" target="mainFrame" onFocus="this.blur()">首页</a>&nbsp;&nbsp;
    	<a href="javascript:post('TakeServlet?action=takeDoSingInRegister',
    	{'idCard':'${sessionScope.idCard }', 'schoolName':'${sessionScope.schoolName }', 
    	'currentPage':'${sessionScope.pageStudent.currentPage-1}'})" target="mainFrame" onFocus="this.blur()">上一页</a>&nbsp;&nbsp;
    	<a href="javascript:post('TakeServlet?action=takeDoSingInRegister',
    	{'idCard':'${sessionScope.idCard }', 'schoolName':'${sessionScope.schoolName }', 
    	'currentPage':'${sessionScope.pageStudent.currentPage+1}'})" target="mainFrame" onFocus="this.blur()">下一页</a>&nbsp;&nbsp;
    	<a href="javascript:post('TakeServlet?action=takeDoSingInRegister',
    	{'idCard':'${sessionScope.idCard }', 'schoolName':'${sessionScope.schoolName }', 
    	'currentPage':'${sessionScope.pageStudent.totalPage}'})" target="mainFrame" onFocus="this.blur()">尾页</a>
    	&nbsp;&nbsp;总共 ${sessionScope.pageStudent.totalCount }条
    </td>
  </tr>
  <tr>
